...
train_df = pd.read_csv(printed_folder)
...
train_df['fold'] = -1
for fold, (train_idx, val_idx) in enumerate(skf.split(X=train_df.image_id, y=train_df["class"])):
    train_df.loc[val_idx, 'fold'] = fold

train_df.to_csv(os.path.join(dst_path, "skf_printed.csv"), index=False)
